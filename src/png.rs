...
use crate::chunk::{Chunk, bytes_to_word};
...
impl TryFrom<&[u8]> for Png {
    ...
    fn try_from(value: &[u8]) -> Result<Self, Self::Error> {
        ...
        while index < n {
            let chunk_len = bytes_to_word(&value[index..index + 4]) as usize;
            chunks.push(Chunk::try_from(&value[index..index + chunk_len + 12])?);
            index += chunk_len + 12;
        }
...
impl Png {
    ...
    pub fn remove_first_chunk(&mut self, chunk_type: &str) -> Result<Chunk, String> {
        for (index, chunk) in self.chunks.iter().enumerate(){
            if chunk_type.as_bytes() == chunk.chunk_type().bytes() {
                return Ok(self.chunks.remove(index))
            }
        }
        Err(String::from("Not such type"))
    }
    ...
    pub fn as_bytes(&self) -> Vec<u8> {
        let mut bytes = Vec::<u8>::from(self.header);
        for chunk in &self.chunks {
